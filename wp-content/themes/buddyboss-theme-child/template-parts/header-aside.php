		...
		<?php if ( is_user_logged_in() ) : ?>
			<div class="user-wrap user-wrap-container menu-item-has-children">
				...
				<a class="user-link" href="<?php echo esc_url( $user_link ); ?>">
					<span class="user-name header-aside-change"><?php echo esc_html( $display_name ); ?></span><i class="bb-icon-angle-down header-aside-change"></i>
			...
			if ( ( class_exists( 'SFWD_LMS' ) && buddyboss_is_learndash_inner() ) || ( class_exists( 'LifterLMS' ) && buddypanel_is_lifterlms_inner() ) ) :
				...
				if ( $show_search || $show_messages || $show_notifications || $show_shopping_cart ) :
					?>
					<span class="bb-separator header-aside-change"></span>
					<?php
				endif;

				if ( $show_search ) :
					?>
					<a href="#" class="header-search-link" data-balloon-pos="down" data-balloon="<?php esc_html_e( 'Search', 'buddyboss-theme' ); ?>"><i class="bb-icon-search header-aside-change"></i></a>
					<?php
				endif;
