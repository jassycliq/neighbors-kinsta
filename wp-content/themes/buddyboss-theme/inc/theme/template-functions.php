	...
	function buddyboss_theme_body_classes( $classes ) {
		...
		if ( ! is_page_template( 'page-fullscreen.php' ) ) {
			if ( $show_buddypanel && has_nav_menu( $menu ) ) {
				...
				// phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
				if ( ( isset( $_COOKIE['buddypanel'] ) && 'open' == $_COOKIE['buddypanel'] ) || ( 'open' == $buddypanel_default_state && ! isset( $_COOKIE['buddypanel'] ) && ! buddyboss_is_learndash_inner() ) ) {
					$classes[] = 'buddypanel-open';
				}
			}

			// phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
			if ( has_nav_menu( $menu ) && '3' == $header ) {
				$classes[] = 'bb-buddypanel';
			...
			// phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
			if ( has_nav_menu( $menu ) && '3' == $header && ! buddyboss_is_learndash_inner() ) {
...
if ( ! function_exists( 'buddypanel_position_right' ) ) {
	...
	function buddypanel_position_right() {
		...
		// phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
		if ( ( $show_buddypanel || '3' == $header ) && $buddypanel_side && 'right' == $buddypanel_side ) {
...
if ( ! function_exists( 'bb_set_buddypanel_status' ) ) {
	...
	function bb_set_buddypanel_status() {
		...
		$cookie_value = ! empty( $_POST['buddypanelStatus'] ) ? sanitize_text_field( wp_unslash( $_POST['buddypanelStatus'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing
